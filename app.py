...
with st.sidebar:
    ...
    if st.button("➕ New Chat"):
        ...
        st.session_state.play_audio_text = None  # Clear audio state
        st.rerun()
    ...
    for idx, chat in enumerate(st.session_state.chats):
        with st.expander(f"{'📌' if chat.get('pinned', False) else ''} {chat.get('name', 'Chat')}"):
            ...
            if c1.button("Open", key=f"open_{idx}"):
                ...
                st.session_state.play_audio_text = None  # Prevent old audio playing on layout change
                st.rerun()
...
# Render message history WITHOUT embedded autoplay tags
...
# This blocks old audio items from starting up during sidebar updates
if st.session_state.play_audio_text and not st.session_state.stop_audio:
    ...
    st.session_state.play_audio_text = None  # Consume it immediately so it won't repeat!
...
if audio_file and not st.session_state.get("last_audio") == audio_file:
    ...
    if client:
        ...
        buffer.name =import streamlit as st
import json
import os
import time
import io
